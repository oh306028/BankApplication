...
function ClientRequests() {
  const [activeRequests, setActiveRequests] = useState<ClientRequestDetails[]>(
    []
  );
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const result = await AccountsService.getClientRequests();
      ...
      console.error(
        "Błąd podczas pobierania wniosków o zostanie klientem:",
        error
      );
    } finally {
      setIsLoading(false);
  ...
  const handleManageRequest = async (
    ...
    try {
      await AccountsService.manageClientRequest(model, request.clientId);
      await fetchData();
    } catch {
